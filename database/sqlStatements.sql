...
-- SELECT a.a_userID, u.u_username as Name, a.a_eventID, e.e_name as Event,
--  a.a_going as going
-- FROM attendance a , users u, event e
-- WHERE a_userID = 1;
--does cartesion product instead without join statements
...
SELECT a.a_userID, u.u_username as Name, a.a_eventID, e.e_name as Event,
 a.a_going as going
FROM attendance a 
JOIN users u ON a.a_userID = u.userID
JOIN event e ON  a.a_eventID = e.eventID
WHERE a_userID = 1;

SELECT 
    u.u_username AS Name,
    e.e_name     AS Event,
    a.a_going    AS going
FROM attendance a
JOIN users u ON a.a_userID = u.userID
JOIN event e ON a.a_eventID = e.eventID
ORDER BY e.eventID, u.u_username;


SELECT  a.a_eventID, count(*) as totalgoing
FROM attendance a 
where a.a_going = 1
and a.a_eventID = 1;
...
SELECT 
    e.eventID,
    e.e_name AS Event,
    COUNT(*) AS total_going
FROM attendance a
JOIN event e ON a.a_eventID = e.eventID
WHERE a.a_going = 1
GROUP BY e.eventID
ORDER BY total_going DESC
LIMIT 1;
